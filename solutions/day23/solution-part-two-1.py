...
class BurrowState:
    ...
    # ====================
    def __init__(self, rows: list, hallway=None):
        ...
        for row, letters in enumerate(rows):
            burrow[row+1] = list('##' + '#'.join(list(letters)) + '##')
        self.burrow = burrow
        self.move_log = []
        self.total_energy = 0
    ...
    def next_moves(self):
        ...
        for row, pos, a_type in movables:
            if row != 0:
                if LETTER_TO_POS[a_type] == pos and all(self.burrow[row_][pos] == LETTER_TO_POS[a_type] for row_ in range(row, SIDE_ROOM_LAST_ROW)):
                    pass
                else:
...
with timebudget('Method 1'):
    test_find_best(ACTUAL_START_STATE, 50000, 44169)

# # for k, v in VERTICAL_DISTANCE.items():
# #     row1, row2 = k
# #     print(v, vertical_distance(row1, row2))